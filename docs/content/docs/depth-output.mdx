...
Depth capture requires dedicated hardware. Before adding a depth output, confirm that the selected device supports it by checking [`CameraDevice.mediaTypes`](/api/react-native-vision-camera/hybrid-objects/CameraDevice#mediatypes):

```ts
const supportsDepth = device.mediaTypes.includes('depth')
```
