...
Depth capture requires dedicated hardware. Before adding a depth output, confirm that the selected device supports it using the [`supportsDepthCapture`](/api/react-native-vision-camera/functions/supportsDepthCapture) helper (or by checking [`CameraDevice.mediaTypes`](/api/react-native-vision-camera/hybrid-objects/CameraDevice#mediatypes) directly):

```ts
import { supportsDepthCapture } from 'react-native-vision-camera'
...
You can also select a depth-capable device directly via [`getCameraDevice`](/api/react-native-vision-camera/functions/getCameraDevice):
...
if (device == null) {
  console.warn('No depth-capable back camera found on this device!')
}
```

> [!NOTE]
> **iOS**: Depth is available on back LiDAR cameras (iPhone 12 Pro and newer) and front TrueDepth cameras (Face ID iPhones).
> **Android**: Depth is available on devices with a Time-of-Flight (ToF) sensor.
> Devices that lack dedicated depth hardware will not expose `'depth'` in `mediaTypes` and attempting to add a `CameraDepthFrameOutput` to such a device will result in a runtime error.
